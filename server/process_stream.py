...
class ProcessStream:
    def __init__(self, model_path, rasa_psa="3", box_iou_thresh=0.55, box_conf_thresh=0.50, kpt_conf_thresh=0.68):
    ...
    def process_frame(self, frame, BOX_IOU_THRESH=0.55, BOX_CONF_THRESH=0.30, KPT_CONF_THRESH=0.68):
        ...
        frames_batch_resized = frames_batch_resized.to(self.device)
        pred_boxes = None

        # Get results from the large model
        results_duzy = self.model(frames_batch_resized, conf=BOX_CONF_THRESH, iou=BOX_IOU_THRESH)
        ...
        if len(self.past_emotions) > 10:
            self.past_emotions.pop(0)

        emotion = self.determine_dominant_emotion(self.past_emotions)

        return frame, pred_boxes, emotion
